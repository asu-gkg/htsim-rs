...
> 当前阶段是“框架原型”：只实现了最小的事件队列、显式路径 `route`、链路的串行化发送（`busy_until`），以及 `dumbbell` 示例。后续你会在这个骨架上逐步加协议、路由、队列模型、统计等。
...
- **网络模型层（`net`）**：只关心“包在网络里怎么走”
  ...
  - `Node`: 节点行为（`Host`/`Switch` 当前都只是“按 route 往下跳”）
...
1. **找到下一跳与链路**：根据 `pkt.next()` 和 `edges[(from,to)]` 找到 `Link`
2. **算发送开始时间**：
   - `now = sim.now()`
...
3. `Network::deliver()` 把目标节点取出来并调用 `node.on_packet(pkt, sim, self)`：
   - 如果 `pkt.has_next()`：继续 `net.forward_from(...)`（再 schedule 下一跳到达事件）
   - 如果没有下一跳：调用 `net.on_delivered(pkt)` 更新统计
...
- `DeliverPacket::execute()`：打印到达时间、到达节点、`pkt.hop`
...
```rust
eprintln!(
    "[t={:?}] fwd pkt={} flow={} from={:?} to={:?} hop={}",
    sim.now(), pkt.id, pkt.flow_id, from, to, pkt.hop
);
```
...
- 想加多流：再 schedule 多个 `InjectFlow`，每个用不同 `flow_id`、不同 route
